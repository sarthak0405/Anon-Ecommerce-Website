const kidmod = require('../Model/kidsmodel')

const getkid = async (req, res) => {

    try {
        const data = await kidmod.find()
...
const deletekid = async (req, res) => {
    ...
    try {
        const data = await kidmod.deleteOne({ name })
...
const postkid = async (req, res) => {
    const { name, img, Category, price } = req.body
    try {
        const data = new kidmod({ name, img, Category, price })
...
const updatekid = async (req, res) => {
    ...
    try {
        ...
        const data = await kidmod.updateOne({ name: req.params.name }, { $set: { name, Category, price, img } })
...
module.exports = { getkid, deletekid, postkid, updatekid }